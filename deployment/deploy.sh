#!/bin/bash
set -ex
echo "deploy.sh 시작 - NEW_COLOR: $NEW_COLOR, DEPLOY_BACKEND: $DEPLOY_BACKEND, DEPLOY_AI: $DEPLOY_AI"
...
if [ "$DEPLOY_BACKEND" == "true" ]; then
  echo "=== backend 배포 시작 ($NEW_COLOR) ==="

  # redis-stream 기동 (없으면 생성, 있으면 유지)
  ...
  echo "backend 배포 완료: $NEW_COLOR 활성화"
else
  echo "=== backend 변경 없음, 배포 스킵 ==="
fi

if [ "$DEPLOY_AI" == "true" ]; then
  echo "=== devchat-ai 배포 시작 ==="
  docker-compose pull dev-chat-ai
  docker-compose up -d --no-deps dev-chat-ai
  echo "devchat-ai 배포 완료"
else
  echo "=== devchat-ai 변경 없음, 배포 스킵 ==="
fi
