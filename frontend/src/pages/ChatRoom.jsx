  ...
  const fetchMessages = useCallback(
    async (cursorValue = null, isLoadMore = false) => {
      ...
      try {
        ...
        const hasMore = nextCursor !== null && messageList.length > 0
  ...
  useEffect(() => {
    ...
    const initializeRoom = async () => {
      ...
      try {
        ...
        const [fetchedRoomId] = await Promise.all([fetchRoomInfo(), fetchCurrentUser()])
  ...
  useEffect(() => {
    ...
    if (!isInitialLoad && messages.length > 0) {
      if (isLoadingMessages) {
        ...
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            // 한 번 더 requestAnimationFrame을 사용하여 확실히 DOM 반영 후
    ...
    return (
      ...
      >
        <div style={{ textAlign: "center" }}>
          <div style={{ color: "#dc3545", marginBottom: "16px" }}>{initState.errorMessage}</div>
          <button
            onClick={() => navigate("/")}
            style={{
              padding: "8px 16px",
